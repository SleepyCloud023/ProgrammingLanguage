...
_global__ void maxpool(float *input, float *output, const int input_size, const int filter_size) {
    ...
    __shared__ float loads[256];
    // out of bound
    // CHANGE
    int output_size = input_size / filter_size;
    float result = - 1000000000;
    if ( row < input_size && col < input_size){
        loads[threadIdx.x + threadIdx.y * 16] = input[row*input_size + col];
    }
    __syncthreads();
    int index = threadIdx.y * 16 + threadIdx.x;
    int bound = input_size - (input_size % filter_size);
    if ( (col % filter_size) == 0 && (row % filter_size) == 0 && col < bound && row < bound){
        for (int i = 0; i < filter_size ; i++){
            for(int j = 0; j < filter_size; j++){
                int index_iter = index + 16 * i + j;
                if (index_iter < 256){
                    float temp = loads[index_iter];
                    if(result < temp){
                        result = temp;
                    }
                }
        ...
        int 
        output[(row/filter_size) * output_size + (col/filter_size)] = result;
    }
}
